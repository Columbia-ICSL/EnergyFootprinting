...
with open('userSnapshots.csv', 'wb') as csvfile:
	...
	writeArray = []
	writeArray += ["year", "month", "day", "hour", "minute", "second", "value", "groundTruth"]
	...
	for shot in shots:
		...
		if userID in userList:
			value = userList[userID]["value"]
			writeArray.append(value)
			consumptions = userList[userID]["consumptions"]
			groundTruth = 0
			for device in consumptions:
				groundTruth += device["share"]
			writeArray.append(groundTruth)
			if (value != groundTruth):
				print((value, groundTruth))
			spamwriter.writerow(writeArray)
#	for person in users:
